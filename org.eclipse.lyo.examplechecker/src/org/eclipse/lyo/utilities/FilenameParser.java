...
public class FilenameParser {
	...
	public String[] getFileNames() {
		...
		if (!this.isDirectory()) {
			results.add(this.pqn);
		}
		else {
			File[] files = new File(Paths.get(filename).toAbsolutePath().toString()).listFiles();
			for (File file : files) {
				if (file.isFile() && file.canRead() ) {
					try {
						results.add(file.getCanonicalPath());
					} catch (IOException e) {
						OSLCToolLogger.error( "Unable to get canonical path for file " + file.getName() + ", in directory " + filename);
